...
-(void) viewDidLoad {
    ...
    ESize s{};
    s.width = static_cast<uint32_t>(self.view.frame.size.width);
    s.height = static_cast<uint32_t>(self.view.frame.size.height);
    
    _eagleExample = new EagleExample(self.view, 1.0f, s);		// SRS - Use 1x scale factor for UIOverlay on iOS
...
-(void) renderFrame {
	//_eagleExample->renderFrame();
	_eagleExample->displayLinkOutputCb();   // SRS - Call displayLinkOutputCb() to animate frames vs. renderFrame() for static image
}
...
-(void) insertText: (NSString*) text {
	...
	_eagleExample->keyPressed(keychar);
}

// The delete backward key has been pressed.
-(void) deleteBackward {
	// _eagleExample->keyPressed(KEY_DELETE);
}
...
-(void) handlePanGesture: (UIPanGestureRecognizer*) gestureRecognizer {
	switch (gestureRecognizer.state) {
		case UIGestureRecognizerStateBegan: {
			...
			_eagleExample->otherMouseDown(_startPoint.x, _startPoint.y);
			break;
		}
		case UIGestureRecognizerStateChanged: {
			...
			_eagleExample->mouseDragged(_startPoint.x + translation.x, _startPoint.y + translation.y);
			break;
		}
		default: {
...
-(void) handlePinchGesture: (UIPinchGestureRecognizer*) gestureRecognizer {
	switch (gestureRecognizer.state) {
		case UIGestureRecognizerStateBegan: {
			_startPoint = [self getGestureLocalPoint: gestureRecognizer];
			_eagleExample->rightMouseDown(_startPoint.x, _startPoint.y);
			break;
		}
		case UIGestureRecognizerStateChanged: {
			_eagleExample->mouseDragged(_startPoint.x, _startPoint.y - self.view.frame.size.height * log(gestureRecognizer.scale));
			break;
		}
		default: {
